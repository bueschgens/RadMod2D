...
function new_figure(;res=(900, 900), xlabel="X in m", ylabel="Y in m")
...
function plot_model(fig, ax, m::AbstractModel; show_norm_vec=false, shownodes=false, 
    ...
    plt_nodes = Vector(undef, m.no_parts)
    plt_com = Vector(undef, m.no_parts)
    plt_norm_vec = Vector(undef, m.no_parts)
    # colors = [:red, :green, :blue, :orange]
    # colors = to_colormap(:rainbow, m.no_parts)
    for p = 1:m.no_parts
        erange = m.elem2par[p].first:m.elem2par[p].last
        # plot elements
        elem_lines[p] = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i in erange]
        plt_elem[p] = linesegments!(ax, elem_lines[p], color=colors[p], linewidth=linewidth)
        # plot nodes
        if shownodes == true
            local nodes_points = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) for i in erange]
            append!(nodes_points, [Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i in erange])
            plt_nodes[p] = scatter!(ax, nodes_points, color=colors[p], markersize=10)
        end
        # plot com
        if showcom == true
            local com_points = [Point2f0(m.elements[i].com.x, m.elements[i].com.y) for i in erange]
            plt_com[p] = scatter!(ax, com_points, color=:grey, markersize=10)
        end
        # plot norm_vec as linesegments
        if show_norm_vec == true
            local com_points = [Point2f0(m.elements[i].com.x, m.elements[i].com.y) for i in erange]
            # local norm_vec_lines = [Point2f0(m.elements[i].com.x, m.elements[i].com.y) => 
            #             Point2f0(m.elements[i].com.x + m.elements[i].norm_vec.x, m.elements[i].com.y + m.elements[i].norm_vec.y) for i in erange]
            ...
            local norm_vec_directions = [Point2f0(m.elements[i].norm_vec.x * scale, m.elements[i].norm_vec.y * scale) for i in erange]
    ...
    # legend
    if showleg == true
        leg_string = ["p-$i: $(m.part_names[i])" for i = 1:m.no_parts ]
        if legpos == "right"
...
function plot_model_elements(fig, ax, m::AbstractModel, erange; show_norm_vec=false, shownodes=false, showcom=false, color=:cyan, linewidth=3.0, linestyle=nothing)
    # plot only given elements
    # color = :cyan
    # plot elements
    elem_lines = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
        Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i in erange]
    ...
    if show_norm_vec == true
        ...
        local norm_vec_directions = [Point2f0(m.elements[i].norm_vec.x * scale, m.elements[i].norm_vec.y * scale) for i in erange]
        lw_arrow = linewidth * 0.5
        arrows!(ax, com_points, norm_vec_directions, arrowcolor=color, arrowsize=arrowsize, linecolor=color, linewidth=lw_arrow)
    end
end
...
function plot_existing(fig, ax, m::AbstractModel, e::T, mat; color=:yellow, linewidth=2) where {T<:Integer}
    ...
    # plot rays
    # color = :yellow
    ray_lines = [Point2f0(m.elements[e].com.x, m.elements[e].com.y) =>
        Point2f0(m.elements[i].com.x, m.elements[i].com.y) for i in exist]
    plt_ray = linesegments!(ax, ray_lines, color=color, linewidth=linewidth)
end


function plot_model_shadow_1to1(fig, ax, m::AbstractModel, mat, source, target)
    ...
    source_lines = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
        Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_source]
    # plot target
    ...
    target_lines = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
        Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_target]
    ...
    for p in rest
        ...
        rest_lines[s1:s2] = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_rest]
    end
    # plot it
...
function plot_model_shadow_1toAll(fig, ax, m::AbstractModel, mat, source)
    # plot specified parts of model with their values in global colormap
    # plot source
    color_source = :yellow
    ns = (m.elem2par[source].last - m.elem2par[source].first + 1)
    ...
    for p in target
        ...
        target_lines[s1:s2] = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange_target]
        k = 0
        for j = s1:s2
            k += 1
...
function plot_model_with_value(fig, ax, m::AbstractModel, val::Vector{T}, val_label; parts=1:m.no_parts, cmap=:jet, linewidth=5.0, showcbar=true, cbarpos="right") where {T<:AbstractFloat}
    # plot specified parts of model with their values in global colormap
    n = 0
    for p in parts
    ...
    for p in parts
        ...
        elem_lines[s1:s2] = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) =>
            Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i = erange]
    end
    plt_elem = linesegments!(ax, elem_lines, color=pval, colormap=cmap, linewidth=linewidth)
    # all at once
    # elem_lines = [Point2f0(m.nodes[m.elements[i].no_node1].x, m.nodes[m.elements[i].no_node1].y) => 
    #             Point2f0(m.nodes[m.elements[i].no_node2].x, m.nodes[m.elements[i].no_node2].y) for i=1:m.no_elements]
